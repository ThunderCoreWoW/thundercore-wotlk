...
function comp_ccacheClean() {
    [ "$TC_CCACHE" != true ] && echo "ccache is disabled" && return
...
function comp_compile() {
  ...
  case "$OSTYPE" in
    msys*)
      ...
      ;;
    linux*|darwin*)
      local confDir=${CONFDIR:-"$TC_BINPATH_FULL/../etc"}

      # create the folders before installing to
      # set the current user and permissions
      echo "Creating $TC_BINPATH_FULL..."
      mkdir -p "$TC_BINPATH_FULL"
      ...
      find "$TC_BINPATH_FULL"  -mindepth 1 -maxdepth 1 -type f -exec sudo chown root:root -- {} +
      find "$TC_BINPATH_FULL"  -mindepth 1 -maxdepth 1 -type f -exec sudo chmod u+s  -- {} +

      if [[ -n "$DOCKER" ]]; then
          [[ -f "$confDir/world.conf.dist" ]] && \
              cp -nv "$confDir/world.conf.dist" "$confDir/world.conf"
          [[ -f "$confDir/login.conf.dist" ]] && \
              cp -nv "$confDir/login.conf.dist" "$confDir/login.conf"
