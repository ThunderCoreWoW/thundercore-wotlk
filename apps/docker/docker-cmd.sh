...
# TODO decide if we need a wrapper like this around docker
# commands.
#
...
> start:app             # Start the development world and login
> start:app:d           # Start the development world and login in detached mode
> build                 # build the development world and login
> pull                  # pull the development world and login
> build:nocache         # build the development world and login without cache
...
> dev:build             # compile thundercore using the dev server
> dev:dash              # execute the dashboard in the dev server container
> dev:shell [ ARGS... ] # open a bash shell in the dev server
> prod:build            # Build the service containers used by tcore-docker
> prod:pull             # Pull the containers used by tcore-docker
> prod:up               # Start the services used by tcore-docker
> prod:up:d             # start the services used by tcore-docker in the background
...
EOF
        ...
        dev:build)
            set -x
            docker compose --profile dev run --rm ac-dev-server bash /thundercore/tcore.sh compiler build
            set +x
            shift
            ;;
        ...
        dev:dash)
            set -x
            docker compose --profile dev run --rm ac-dev-server bash /thundercore/tcore.sh ${@:2}
            set +x
            shift
            ;;
