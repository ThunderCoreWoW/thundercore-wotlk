function inst_configureOS() {
    ...
    case "$OSTYPE" in
        ...
        darwin*)  source "$TC_PATH_INSTALLER/includes/os_configs/osx.sh" ;;
        linux*)
            ...
            source "$TC_PATH_INSTALLER/includes/os_configs/$DISTRO.sh"
        ;;
        bsd*)     echo "BSD is not supported yet" ;;
        msys*)    source "$TC_PATH_INSTALLER/includes/os_configs/windows.sh" ;;
...
function inst_getVersionBranch() {
    ...
    local parsed=$(echo "$curldata" | "$TC_PATH_DEPS/jsonpath/JSONPath.sh" -b '$.compatibility.*.[version,branch]')

    semverParseInto "$TCORE_VERSION" TCV_MAJOR TCV_MINOR ACV_PATCH TCV_SPECIAL
    ...
    if [[ ! -z "$parsed" ]]; then
        ...
        for idx in `seq 0 $((${#vers[*]}/2-1))`; do
            ...
            if [[ $MODULE_MAJOR -eq $TCV_MAJOR && $MODULE_MINOR -le $TCV_MINOR ]]; then
                res="${vers[idx*2+1]}"
                v="${vers[idx*2]}"
            fi
        done
...
function inst_module_search {
    ...
    readarray -t MODS < <(curl --silent "https://api.github.com/search/repositories?q=org%3ThunderCoreWoW${search}+fork%3Atrue+topic%3Acore-module+sort%3Astars&type=" \
        | "$TC_PATH_DEPS/jsonpath/JSONPath.sh" -b '$.items.*.name')
    while (( ${#MODS[@]} > idx )); do
        mod="${MODS[idx++]}"
        read v b < <(inst_getVersionBranch "https://raw.githubusercontent.com/ThunderCoreWoW/$mod/master/tcore-module.json")

        if [[ "$b" != "none" ]]; then
            echo "-> $mod (tested with TC version: $v)"
        else
            echo "-> $mod (no revision available for TC v$TC_VERSION, it could not work!)"
        fi
    done
...
function inst_module_install {
    ...
    read v b < <(inst_getVersionBranch "https://raw.githubusercontent.com/ThunderCoreWoW/$res/master/tcore-module.json")

    if [[ "$b" != "none" ]]; then
        Joiner:add_repo "https://github.com/ThunderCoreWoW/$res" "$res" "$b" && echo "Done, please re-run compiling and db assembly. Read instruction on module repository for more information"
    else
        echo "Cannot install $res module: it doesn't exists or no version compatible with TC v$TCORE_VERSION are available"
    fi
...
function inst_module_update {
    ...
    if [ -d "$J_PATH_MODULES/$res/" ]; then
        read v b < <(inst_getVersionBranch "https://raw.githubusercontent.com/thundercore/$res/master/tcore-module.json")
        ...
        Joiner:upd_repo "https://github.com/ThunderCoreWoW/$res" "$res" "$b" && echo "Done, please re-run compiling and db assembly" || echo "Cannot update"
        cd $_tmp
    else
...
function inst_simple_restarter {
    echo "Running $1 ..."
    bash "$TC_PATH_APPS/startup-scripts/simple-restarter" "$TC_BINPATH_FULL" "$1"
    echo
    #disown -a
    #jobs -l
}
...
function inst_download_client_data {
    ...
    local path="${DATAPATH:-$TC_BINPATH_FULL}"
