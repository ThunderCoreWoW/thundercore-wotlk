...
options=(
    ...
    "run-login (ra): execute a simple restarter for login" # 12
...
function _switch() {
    ...
    case $_reply in
        ...
        ""|"r"|"reset"|"4")
            ...
            ;;
        ""|"c"|"compiler"|"5")
            bash "$TC_PATH_APPS/compiler/compiler.sh" $_opt
            ;;
        ""|"ms"|"module-search"|"6")
        ...
        ""|"gd"|"client-data"|"10")
            ...
            ;;
        ""|"rw"|"run-world"|"11")
            inst_simple_restarter world
            ;;
        ""|"ra"|"run-login"|"12")
            inst_simple_restarter login
            ;;
        ""|"dr"|"docker"|"13")
            DOCKER=1 bash "$TC_PATH_ROOT/apps/docker/docker-cmd.sh" "${@:2}"
            exit
            ;;
        ""|"v"|"version"|"14")
            # denoRunFile "$TC_PATH_APPS/installer/main.ts" "version"
            printf "ThunderCore Rev. %s\n" "$TCORE_VERSION"
            exit
            ;;
        ""|"quit"|"15")
            echo "Goodbye!"
