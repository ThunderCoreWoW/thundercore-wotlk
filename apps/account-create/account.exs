...
#
# $  TCORE_USERNAME=foo TCORE_PASSWORD=barbaz123 elixir account.exs
#
...
# TCORE_GM_LEVEL - GM level for account 
...
username_lower =
  System.get_env("TCORE_USERNAME", default_credential)
  |> tap(&Logger.info("Account to create: #{&1}"))

username = String.upcase(username_lower)

password = System.get_env("TCORE_PASSWORD", default_credential)

gm_level = System.get_env("TCORE_GM_LEVEL", default_gm_level) |> String.to_integer()

if Range.new(0, 3) |> Enum.member?(gm_level) |> Kernel.not do
  Logger.info("Valid TCORE_GM_LEVEL values are 0, 1, 2, and 3. The given value was: #{gm_level}.")
end

{:ok, pid} =
  MyXQL.start_link(
    protocol: :tcp,
    database: System.get_env("MYSQL_DATABASE", "thunder_auth"),
