...
if (!empty($_FILES['File1']) && !empty($_FILES['File2']))
{
    ...
    if (strpos($upload1, "world") !== false)
        $newconfig = $basedir."/".session_id()."/world.conf.merge";
    else if (strpos($upload1, "login") !== false)
        $newconfig = $basedir."/".session_id()."/login.conf.merge";
    else
