...
SRCPATH="$TC_PATH_ROOT"

# absolute path where build files must be stored
BUILDPATH=${BUILDPATH:-"$TC_PATH_VAR/build/obj"}

# absolute path where thundercore will be installed
# NOTE: on linux the binaries are stored in a subfolder (/bin)
# of the $BINPATH
BINPATH="$TC_PATH_ROOT/env/dist"
...
# CONFDIR="$TC_PATH_ROOT/env/dist/etc/"
...
# thundercore's official remote source address to pull from
...
ORIGIN_REMOTE="https://github.com/ThunderCoreWoW/thundercore-wotlk.git"
...
# recompilations
#
TC_CCACHE=${TC_CCACHE:-false}
export CCACHE_DIR=${CCACHE_DIR:-"$TC_PATH_VAR/ccache"}
...
#  4. run "killall -12 worldr"  This command will start the monitoring process. Run "killall -12 world" again to stop the process when you want
#  5. At this time you will have the .prof file ready in the folder configured below.
#  Run "google-pprof --callgrind <path/of/world/bin> </path/of/prof/file>" This will generate a callgrind file that can be read with
#  QCacheGrind, KCacheGrind and any other compatible tools
#
##############################################

# files used by gperftools to store monitored information
export CPUPROFILE=${CPUPROFILE:-"$BINPATH/logs/world-cpu.prof"}
# heap profile is disabled by default. Uncomment this line to enable it
# export HEAPPROFILE=${HEAPPROFILE:-"$BINPATH/logs/world-heap.prof"}
